...
ARG _RESTY_CONFIG_DEPS="--add-module=/tmp/ngx_pagespeed-latest-stable --with-openssl=/tmp/openssl-${RESTY_OPENSSL_VERSION} --with-pcre=/tmp/pcre-${RESTY_PCRE_VERSION}"
...
RUN \
    ...
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        ...
        zlib1g-dev \
    && cd /tmp \
    && curl -f -L -sS https://ngxpagespeed.com/install -o install \
    && bash install -b /tmp
    RUN \
    && cd /tmp \
